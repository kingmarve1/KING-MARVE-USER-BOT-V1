...
class KingMarveBot:
    ...
    async def start_bot(self):
        ...
        try:
            ...
            logger.info("👑 𝗖𝗿𝗲𝗮𝘁𝗼𝗿: KING MARVE")
            ...
            # Keep the bot running without restarting
            await self.client.run_until_disconnected()
            
        except Exception as e:
            logger.error(f"Error starting bot: {e}")
            # Don't auto-restart, just wait for manual restart
            await asyncio.sleep(300)  # Wait 5 minutes before exiting
            return

    async def register_handlers(self):
        """Register all event handlers"""
        
        # Handle /connect with phone number in one command
        @self.client.on(events.NewMessage(pattern=r'/connect(?:\s+(\+?\d{10,15}))?'))
        async def connect_handler(event):
            match = event.pattern_match
            if match.group(1):  # If phone number is provided with /connect
                phone_number = match.group(1).strip()
                if not phone_number.startswith('+'):
                    phone_number = '+' + phone_number
                await self.handle_connect_complete(event, phone_number)
            else:
                await self.handle_connect(event)
        
        # Handle /verify with code in one command
        @self.client.on(events.NewMessage(pattern=r'/verify(?:\s+(\d+))?'))
        async def verify_handler(event):
            match = event.pattern_match
            if match.group(1):  # If code is provided with /verify
                code = match.group(1).strip()
                await self.handle_verify_direct(event, code)
            else:
        ...
        # Handle phone number as separate message
        @self.client.on(events.NewMessage())
        async def message_handler(event):
            user_id = event.sender_id
            # Check if it's a phone number message
            if re.match(r'^\+?\d{10,15}$', event.text.strip()) and user_id in self.waiting_for_phone:
        ...
        if user_id not in self.user_sessions:
            await event.reply("❌ You need to connect first! Use `/connect +1234567890`")
            return False
        return True
...
🤖 **To get started:**
Use one command: `/connect +2347067955105`

💡 **Quick commands:**
• `/connect +2347067955105` - Connect with phone
• `/verify 12345` - Verify with code
...
🌐 **Bot features 24/7 uptime on cloud hosting!**
    ...
    async def handle_connect(self, event):
        """Handle /connect without phone number"""
        user_id = event.sender_id
        ...
        await event.reply(
            ...
            "**Format:** `/connect +2347067955105`\n"
            "**Or send just:** `+2347067955105`\n\n"
            "⚠️ **Include country code, no spaces**"
        )

    async def handle_connect_complete(self, event, phone_number):
        """Handle complete connection process"""
        user_id = event.sender_id
        
        # Validate phone number
        if not re.match(r'^\+\d{10,15}$', phone_number):
            await event.reply("❌ Invalid phone number format.\n\n**Example:** `+2347067955105`")
            return
        ...
        try:
            ...
            await event.reply(
                ...
                "Use: `/verify 12345`\n\n"
                "⚠️ **Enter code quickly to avoid expiration!**"
            )
            
        except Exception as e:
            error_msg = str(e)
            if "FLOOD" in error_msg:
                await event.reply("❌ Too many attempts. Please wait 5 minutes.")
            elif "PHONE_NUMBER_INVALID" in error_msg:
                await event.reply("❌ Invalid phone number. Check format: `+2347067955105`")
            elif "PHONE_NUMBER_OCCUPIED" in error_msg:
                await event.reply("❌ Phone number already in use.")
            else:
    ...
    async def handle_verify_direct(self, event, verification_code):
        """Handle verification with direct code"""
        user_id = event.sender_id
        
        if user_id not in self.waiting_for_code:
            await event.reply("❌ No verification pending. Use `/connect +1234567890` first.")
            return
        
        user_data = self.waiting_for_code[user_id]
        
        # Check if code expired (5 minutes)
        if time.time() - user_data['timestamp'] > 300:
            await event.reply("❌ Verification code expired. Please use `/connect +1234567890` again")
        ...
        try:
            ...
            if "code expired" in error_msg.lower():
                await event.reply("❌ Code expired. Use `/connect +1234567890` again")
                del self.waiting_for_code[user_id]
            elif "PHONE_CODE_EXPIRED" in error_msg:
                await event.reply("❌ Code expired. Use `/connect +1234567890` again")
                del self.waiting_for_code[user_id]
            elif "PHONE_CODE_INVALID" in error_msg:
                await event.reply("❌ Invalid code. Check and try `/verify CODE` again")
            elif "SESSION_PASSWORD_NEEDED" in error_msg:
                await event.reply("❌ 2FA password required. This bot doesn't support 2FA yet.")
            else:
                await event.reply(f"❌ Verification failed: {error_msg}")
...
if __name__ == "__main__":
    print("🌑 Starting 𝐊𝐈N𝐆 𝐌A𝐑𝐕E 𝐔𝐒𝐄𝐑 𝐁𝐎𝐓 on cloud...")
    asyncio.run(main())
